...
Create a new integration package called `$ARGUMENTS` in the `tmp/integrations/` monorepo directory, following the established OpenCompany integration package pattern.

## Before you start

Read the reference implementation to understand the exact patterns:

1. Read `tmp/integrations/celestial/src/CelestialToolProvider.php` — ToolProvider with atomic tool entries
2. Read `tmp/integrations/celestial/src/CelestialServiceProvider.php` — ServiceProvider pattern
3. Read `tmp/integrations/celestial/src/Tools/CelestialMoonPhase.php` — Atomic tool class (one operation per class)
4. Read `tmp/integrations/celestial/composer.json` — Package dependencies
5. Read `tmp/integrations/core/src/Contracts/Tool.php` — The Tool contract to implement
6. Read `tmp/integrations/core/src/Contracts/ToolProvider.php` — The ToolProvider contract
7. Read `tmp/integrations/core/src/Contracts/CredentialResolver.php` — For tools needing API keys
8. Read `tmp/integrations/core/src/Contracts/ConfigurableIntegration.php` — Config UI contract for integrations with credentials

## Package structure to create

```
tmp/integrations/$ARGUMENTS/
├── composer.json
├── LICENSE          (MIT, copy from tmp/integrations/celestial/LICENSE)
├── README.md
...
See `tmp/integrations/clickup/src/ClickUpToolProvider.php` for a complete real-world example.
...
See `tmp/integrations/clickup/lua-docs/clickup.md` for an example.

## After creating the package

1. The monorepo wildcard path (`tmp/integrations/*`) auto-discovers the new package.
