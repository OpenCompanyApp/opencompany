...
const buildCodeSignature = (tool: ToolEntry, namespace: string) => {
  ...
  const params = (tool.parameters || []).map(p => p.required ? p.name + ':' : p.name + ': optional')
...
const highlightSignature = (sig: string): string => {
  ...
  if (!match) return escape(sig)

  const [, ns, fn, paramsStr] = match

  const nsHtml = ns.split('.').map(s =>
    `<span class="sig-ns">${escape(s)}</span>`
  ).join('<span class="sig-dot">.</span>')

  const paramsHtml = paramsStr
    ? paramsStr.split(/,\s*/).map(p => {
        const optional = p.endsWith(': optional')
        const name = escape(p)
        return optional
          ? `<span class="sig-param sig-optional">${name}</span>`
...
/* Ruby signature syntax highlighting (v-html needs :deep). */
