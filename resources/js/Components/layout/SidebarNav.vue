<template>
  ...
  >
    ...
    <div class="space-y-0.5">
      ...
      >
        <Icon
          :name="isActive(dashboardUrl()) ? 'ph:house-fill' : 'ph:house'"
...
const agentWorkItems = computed<NavItemType[]>(() => [
  { to: tasksUrl(), icon: 'ph:check-square', iconActive: 'ph:check-square-fill', label: 'Tasks' },
  { to: approvalsUrl(), icon: 'ph:seal-check', iconActive: 'ph:seal-check-fill', label: 'Approvals', badge: 3 },
  { to: orgUrl(), icon: 'ph:tree-structure', iconActive: 'ph:tree-structure-fill', label: 'Organization' },
  { to: automationUrl(), icon: 'ph:lightning', iconActive: 'ph:lightning-fill', label: 'Automation' },
])

// Office - Daily productivity tools
const officeItems = computed<NavItemType[]>(() => [
  { to: chatUrl(), icon: 'ph:chat-circle', iconActive: 'ph:chat-circle-fill', label: 'Chat', badge: 15 },
  { to: docsUrl(), icon: 'ph:file-text', iconActive: 'ph:file-text-fill', label: 'Docs' },
  { to: filesUrl(), icon: 'ph:folder', iconActive: 'ph:folder-fill', label: 'Files' },
  { to: tablesUrl(), icon: 'ph:table', iconActive: 'ph:table-fill', label: 'Tables' },
  { to: calendarUrl(), icon: 'ph:calendar', iconActive: 'ph:calendar-fill', label: 'Calendar' },
  { to: listsUrl(), icon: 'ph:kanban', iconActive: 'ph:kanban-fill', label: 'Lists' },
])

// Methods
const isActive = (path: string): boolean => {
  const currentUrl = page.url
  if (path === dashboardUrl()) return currentUrl === dashboardUrl()
