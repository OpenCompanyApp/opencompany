...
const sizeConfig: Record<ItemSize, SizeConfig> = {
  ...
  md: {
    container: 'gap-3 px-3 py-2.5 rounded-lg',
  ...
  lg: {
    container: 'gap-4 px-4 py-3 rounded-lg',
...
const highlightText = (text: string, query: string): string => {
  ...
  return text.replace(regex, '<mark class="bg-yellow-100 text-inherit rounded px-0.5">$1</mark>')
}
