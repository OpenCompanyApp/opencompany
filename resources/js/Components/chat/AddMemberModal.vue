<template>
  <DialogRoot :open="open" @update:open="emit('update:open', $event)">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-800 rounded-lg shadow-xl z-50 w-full max-w-md max-h-[80vh] flex flex-col"
      >
        <DialogTitle class="text-lg font-semibold text-white p-4 border-b border-gray-700 flex items-center justify-between">
          <span>Add Members to #{{ channel?.name }}</span>
        ...
        <div class="flex-1 overflow-y-auto px-4 pb-4">
          ...
          <div v-else-if="filteredUsers.length === 0" class="text-center py-8 text-gray-400">
            <Icon name="heroicons:user-group" class="w-12 h-12 mx-auto mb-2 opacity-50" />
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="relative flex-shrink-0">
                ...
                >
                  <Icon name="heroicons:cpu-chip" class="w-2.5 h-2.5 text-white" />
                </div>
              </div>

              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-white truncate">{{ user.name }}</div>
                <div class="text-xs text-gray-400 capitalize">{{ user.type }}</div>
              </div>
              ...
              >
                <Icon name="heroicons:check" class="w-3.5 h-3.5 text-white" />
              </div>
              <div
                v-else
                class="w-5 h-5 border border-gray-500 rounded-full"
              />
            </button>
          </div>
        </div>
