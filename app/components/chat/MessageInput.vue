  ...
  <div :class="containerClasses">
    ...
    <Transition name="slide-up">
      <div v-if="replyTo" :class="replyPreviewClasses">
        <div class="flex items-center gap-2 flex-1 min-w-0">
          <div class="w-0.5 h-full bg-olympus-primary rounded-full shrink-0" />
          <div class="flex-1 min-w-0">
            <p class="text-xs font-medium text-olympus-primary">
              Replying to {{ replyTo.author.name }}
            </p>
            <p class="text-xs text-olympus-text-muted truncate">
              {{ replyTo.content }}
            </p>
          </div>
        </div>
    ...
    <Transition name="slide-up">
      <div v-if="editMessage" :class="editBannerClasses">
        <div class="flex items-center gap-2">
          <Icon name="ph:pencil-simple" class="w-4 h-4 text-amber-400" />
          <span class="text-xs font-medium text-amber-400">Editing message</span>
        </div>
        <button
          type="button"
          ...
          >
            ...
            <template v-else>
              <div class="flex flex-col items-center justify-center h-full p-2">
                ...
                <span class="text-[10px] text-olympus-text-muted truncate w-full text-center mt-1">
                  {{ attachment.name }}
                </span>
              </div>
            </template>
            ...
            <div
              ...
              class="absolute inset-0 bg-olympus-bg/80 rounded-lg flex items-center justify-center"
            >
              <div class="w-8 h-8 relative">
                ...
                <span class="absolute inset-0 flex items-center justify-center text-[10px] font-medium text-olympus-primary">
            ...
            <button
              v-if="!attachment.uploading"
              type="button"
              class="absolute -top-1.5 -right-1.5 w-5 h-5 bg-olympus-elevated border border-olympus-border rounded-full flex items-center justify-center text-olympus-text-muted hover:text-red-400 hover:border-red-400 transition-colors shadow-sm"
    ...
    <div :class="inputAreaClasses">
      ...
      <div class="flex-1 min-w-0 relative">
          ...
          >
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <span v-if="user.type === 'agent'" class="text-xs text-olympus-text-muted">Agent</span>
              </div>
            </button>
          </div>
        </Transition>
        ...
        <Transition name="fade-scale">
          ...
          >
            <div class="text-xs font-medium text-olympus-text-muted uppercase tracking-wider px-3 py-2">
              Commands
            </div>
            <button
            ...
            >
              <Icon :name="command.icon" class="w-4 h-4 text-olympus-text-muted" />
              <div class="flex-1 min-w-0">
                <span class="text-sm text-olympus-text font-medium">{{ command.name }}</span>
                <span class="text-xs text-olympus-text-muted ml-2">{{ command.description }}</span>
              </div>
            </button>
          </div>
        </Transition>
        ...
        <TooltipProvider v-if="showVoiceRecording" :delay-duration="300">
          <TooltipRoot>
            <TooltipTrigger as-child>
              <button
                type="button"
                :class="[actionButtonClasses, isRecording && 'text-red-400 animate-pulse']"
                :disabled="sending"
    ...
    <div :class="footerClasses">
      <div class="flex items-center gap-3">
        <!-- Keyboard Hints -->
        <span v-if="showKeyboardHints" class="text-xs text-olympus-text-subtle">
...
const containerClasses = computed(() => {
  const classes = [
    'border-t border-olympus-border shrink-0 bg-olympus-bg',
...
const replyPreviewClasses = computed(() => [
  'flex items-center gap-2 mb-2 px-3 py-2 bg-olympus-surface rounded-lg',
])

// Edit banner classes
const editBannerClasses = computed(() => [
  'flex items-center justify-between mb-2 px-3 py-2 bg-amber-500/10 border border-amber-500/30 rounded-lg',
])

// Attachments preview classes
const attachmentsPreviewClasses = computed(() => [
  'mb-3 px-3 py-2 bg-olympus-surface rounded-lg',
])

// Attachment item classes
const attachmentItemClasses = computed(() => [
  'relative w-20 h-20 bg-olympus-elevated rounded-lg shrink-0 overflow-hidden',
  'border border-olympus-border',
])

// Add attachment button classes
const addAttachmentButtonClasses = computed(() => [
  'w-20 h-20 flex items-center justify-center shrink-0',
  'bg-olympus-surface hover:bg-olympus-elevated rounded-lg',
  'border-2 border-dashed border-olympus-border hover:border-olympus-primary',
...
const inputAreaClasses = computed(() => {
  const classes = [
    'flex items-end gap-3 bg-olympus-surface rounded-lg border transition-all duration-200',
    sizeConfig[props.size].input,
  ]

  if (isFocused.value) {
    classes.push('border-olympus-primary shadow-glow-sm')
  } else {
    classes.push('border-olympus-border')
  }
...
const formattingToolbarClasses = computed(() => [
  'absolute bottom-full left-0 mb-2 flex items-center gap-1 px-2 py-1',
...
const textareaClasses = computed(() => [
  'w-full bg-transparent outline-none text-olympus-text',
  'placeholder:text-olympus-text-subtle resize-none max-h-32',
...
const actionButtonClasses = computed(() => [
  'rounded-md text-olympus-text-muted hover:text-olympus-text',
...
const sendButtonClasses = computed(() => {
  const classes = [
    'ml-2 rounded-lg font-medium transition-all duration-150 outline-none',
    'focus-visible:ring-2 focus-visible:ring-olympus-primary/50',
    'focus-visible:ring-offset-2 focus-visible:ring-offset-olympus-surface',
    sizeConfig[props.size].sendButton,
  ]

  if (canSend.value) {
    classes.push(
      'bg-olympus-primary hover:bg-olympus-primary-hover text-white',
      'active:scale-[0.98] shadow-lg shadow-olympus-primary/25 hover:shadow-olympus-primary/40',
    )
  } else {
    classes.push('bg-olympus-elevated text-olympus-text-subtle cursor-not-allowed')
  }
...
const mentionsPopupClasses = computed(() => [
  ...
  'bg-olympus-elevated border border-olympus-border rounded-xl shadow-xl',
  'z-50',
])
...
const mentionItemClasses = (selected: boolean) => [
  ...
  selected
    ...
    : 'text-olympus-text-muted hover:bg-olympus-surface',
]
...
const kbdClasses = computed(() => [
  'px-1.5 py-0.5 bg-olympus-surface border border-olympus-border rounded text-[10px] font-mono',
])
...
const characterCountClasses = computed(() => {
  ...
  if (percentage >= 1) {
    ...
    classes.push('text-amber-400')
  } else {
    classes.push('text-olympus-text-subtle')
  }
...
const fileIconClasses = (type: string) => {
  ...
  if (type.startsWith('image/')) return [baseClasses, 'text-green-400']
  if (type.startsWith('video/')) return [baseClasses, 'text-purple-400']
  if (type.startsWith('audio/')) return [baseClasses, 'text-amber-400']
  if (type.includes('pdf')) return [baseClasses, 'text-red-400']
  return [baseClasses, 'text-olympus-text-muted']
}
  ...
  setup(_, { emit: pickerEmit }) {
    ...
    return () => h('div', { class: 'w-72 max-h-64 overflow-y-auto p-2' },
      categories.map(category => h('div', { key: category.name, class: 'mb-3' }, [
        h('p', { class: 'text-xs font-medium text-olympus-text-muted uppercase tracking-wider mb-2 px-1' }, category.name),
        h('div', { class: 'grid grid-cols-8 gap-1' },
          category.emojis.map(emoji =>
            h('button', {
              key: emoji,
              type: 'button',
              class: 'w-8 h-8 flex items-center justify-center text-lg hover:bg-olympus-surface rounded transition-colors',
