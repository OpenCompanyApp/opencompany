...
const containerClasses = computed(() => [
  'flex items-center text-olympus-text-muted',
  sizeConfig[props.size].container,
  sizeConfig[props.size].gap,
  props.variant === 'minimal' ? 'bg-transparent' : 'bg-olympus-surface/50 rounded-xl',
...
const namesClasses = computed(() => [
  'font-medium text-olympus-text truncate',
  sizeConfig[props.size].text,
])

// Action classes
const actionClasses = computed(() => [
  'text-olympus-text-muted whitespace-nowrap',
  sizeConfig[props.size].text,
])
...
const dotClasses = computed(() => [
  'rounded-full bg-olympus-text-muted animate-typing-dot',
  sizeConfig[props.size].dot,
])

// Wave bar classes
const waveBarClasses = computed(() => [
  'w-[3px] bg-olympus-text-muted rounded-full animate-typing-wave',
])
...
const pulseInnerClasses = computed(() => [
  'animate-ping absolute inline-flex h-full w-full rounded-full bg-olympus-primary opacity-75',
])
...
const activityIconClasses = computed(() => {
  ...
  const colorMap: Record<ActivityType, string> = {
    typing: 'text-olympus-text-muted',
    recording: 'text-red-400 animate-pulse',
    uploading: 'text-blue-400',
    thinking: 'text-amber-400 animate-pulse',
  }

  return [sizeMap[props.size], colorMap[props.activityType]]
})
...
const dismissButtonClasses = computed(() => [
  ...
  'text-olympus-text-subtle hover:text-olympus-text',
