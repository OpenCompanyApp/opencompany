  ...
  <aside :class="containerClasses">
    ...
    <Transition name="slide-down">
      <div v-if="filterOpen" :class="filterPanelClasses">
        <div class="space-y-3">
          <div>
            <label class="text-xs font-medium text-olympus-text-muted uppercase tracking-wider mb-2 block">
              Channel Type
            </label>
          ...
          <div class="flex items-center justify-between pt-2 border-t border-olympus-border">
            <button
              type="button"
    ...
    <div v-if="showFooter" :class="footerClasses">
      ...
      <label v-if="archivedChannels.length > 0" class="flex items-center gap-2 text-xs text-olympus-text-muted cursor-pointer mt-2">
        <input
          v-model="showArchived"
          type="checkbox"
          class="w-3.5 h-3.5 rounded text-olympus-primary bg-olympus-surface border-olympus-border focus:ring-olympus-primary/50"
        >
...
const containerClasses = computed(() => {
  const classes = [
    'bg-olympus-bg border-r border-olympus-border flex flex-col shrink-0',
    props.width || sizeConfig[props.size].container,
  ]

  if (props.variant === 'floating') {
    classes.push('absolute left-0 top-0 h-full shadow-xl z-10')
  }
...
const headerTitleClasses = computed(() => {
  ...
  return ['font-semibold text-olympus-text', sizeMap[props.size]]
})
...
const filterChipClasses = (active: boolean) => [
  ...
  active
    ? 'bg-olympus-primary/20 text-olympus-primary'
    : 'bg-olympus-surface text-olympus-text-muted hover:text-olympus-text',
]
...
const footerButtonClasses = computed(() => [
  ...
  'text-sm text-olympus-text-muted',
  'hover:text-olympus-text hover:bg-olympus-surface',
  'transition-colors duration-150',
  'focus:outline-none focus-visible:ring-2 focus-visible:ring-olympus-primary/50',
])
    ...
    const headerContent = () => [
      h('div', { class: 'flex items-center gap-2' }, [
        sectionProps.icon && h(resolveComponent('Icon'), {
          ...
          class: ['w-3.5 h-3.5', sectionProps.muted ? 'text-olympus-text-subtle' : 'text-olympus-text-muted'],
        }),
        h('span', {
          class: ['text-xs font-semibold uppercase tracking-wider', sectionProps.muted ? 'text-olympus-text-subtle' : 'text-olympus-text-muted'],
        }, sectionProps.title),
        ...
        sectionProps.showOnlineCount && onlineCount.value > 0 && h('span', {
          ...
          h('span', { class: 'w-1.5 h-1.5 rounded-full bg-green-400' }),
          `${onlineCount.value}`,
        ]),
      ]),
      h('div', { class: 'flex items-center gap-1' }, [
        sectionProps.action && h('button', {
          ...
          class: [
            ...
            'text-olympus-text-muted hover:text-olympus-primary hover:bg-olympus-primary/10',
            'focus:outline-none focus-visible:ring-2 focus-visible:ring-olympus-primary/50',
        ...
        sectionProps.collapsible && h(resolveComponent('Icon'), {
          name: 'ph:caret-down',
          class: [
            'w-3.5 h-3.5 text-olympus-text-muted transition-transform duration-200',
