  ...
  <div :class="containerClasses">
        ...
        >
          ...
          <div
            :class="[
              ...
              action.bgClass || getDefaultBgClass(index),
              (hoveredAction === (action.id || action.label)) && 'scale-110',
          ...
          >
            <Icon
              v-if="action.loading"
              name="ph:spinner"
              :class="[iconClasses, action.iconClass || getDefaultIconClass(index), 'animate-spin']"
            />
            <Icon
              v-else
              :name="action.icon"
              :class="[iconClasses, action.iconClass || getDefaultIconClass(index)]"
            />
          </div>
          ...
          <div class="flex-1 min-w-0 text-left">
            <div class="flex items-center gap-2">
              ...
              <span
                v-if="action.isNew"
                class="px-1.5 py-0.5 text-[9px] font-semibold rounded bg-olympus-primary/20 text-olympus-primary"
              >
                NEW
              </span>
              ...
              <span
                v-if="action.isBeta"
                class="px-1.5 py-0.5 text-[9px] font-semibold rounded bg-amber-500/20 text-amber-400"
              >
                BETA
              </span>
            ...
            <div v-if="showUsageStats && action.usageCount !== undefined" class="mt-1 flex items-center gap-1.5">
              <div class="h-1 flex-1 bg-olympus-bg rounded-full overflow-hidden max-w-20">
                <div
                  class="h-full bg-olympus-primary/50 rounded-full"
                  :style="{ width: `${Math.min((action.usageCount / maxUsageCount) * 100, 100)}%` }"
                />
              </div>
              <span class="text-[9px] text-olympus-text-subtle">
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <span
              ...
              class="text-[10px] text-olympus-text-subtle"
            >
          ...
          >
            <Icon name="ph:push-pin-fill" class="w-3 h-3 text-amber-400" />
          </div>
        </button>
      </TransitionGroup>
    ...
    <div v-if="showRecent && recentActions.length > 0" :class="recentSectionClasses">
      <div class="flex items-center justify-between mb-2">
        <span class="text-[10px] uppercase tracking-wide text-olympus-text-subtle font-medium">
          Recent
        </span>
        <button
          type="button"
          class="text-[10px] text-olympus-text-subtle hover:text-olympus-text"
      ...
      <Transition name="modal">
        <div v-if="showCustomize" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div :class="modalClasses">
            <div class="flex items-center justify-between p-4 border-b border-olympus-border">
              <h3 class="font-semibold">Customize Quick Actions</h3>
              <button
                type="button"
                class="p-1.5 rounded-lg hover:bg-olympus-surface"
            ...
            <div class="p-4 max-h-96 overflow-y-auto">
              <p class="text-sm text-olympus-text-muted mb-4">
              ...
              <div class="space-y-2">
                ...
                >
                  <Icon name="ph:dots-six-vertical" class="w-4 h-4 text-olympus-text-subtle cursor-grab" />
                  <div :class="[iconContainerClasses, action.bgClass, 'w-8 h-8']">
                    <Icon :name="action.icon" :class="['w-4 h-4', action.iconClass]" />
                  </div>
                  <div class="flex-1">
                    <p class="text-sm font-medium">{{ action.label }}</p>
                    <p class="text-xs text-olympus-text-muted">{{ action.description }}</p>
                  </div>
                  <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div class="w-9 h-5 bg-olympus-bg rounded-full peer peer-checked:bg-olympus-primary transition-colors" />
            ...
            <div class="flex justify-end gap-2 p-4 border-t border-olympus-border">
              <SharedButton variant="secondary" size="sm" @click="showCustomize = false">
                Cancel
              </SharedButton>
              <SharedButton size="sm" @click="saveCustomization">
                Save
              </SharedButton>
            </div>
          </div>
...
type QuickActionsSize = 'sm' | 'md' | 'lg'
...
// Color palette for default styling
const defaultColors = [
  { bg: 'bg-blue-500/20', icon: 'text-blue-400' },
  { bg: 'bg-cyan-500/20', icon: 'text-cyan-400' },
  { bg: 'bg-green-500/20', icon: 'text-green-400' },
  { bg: 'bg-amber-500/20', icon: 'text-amber-400' },
  { bg: 'bg-purple-500/20', icon: 'text-purple-400' },
  { bg: 'bg-pink-500/20', icon: 'text-pink-400' },
]

const getDefaultBgClass = (index: number) => defaultColors[index % defaultColors.length].bg
const getDefaultIconClass = (index: number) => defaultColors[index % defaultColors.length].icon
...
const containerClasses = computed(() => [
  'card-gradient bg-olympus-surface rounded-xl',
...
const headerIconClasses = computed(() => [
  'rounded-lg flex items-center justify-center bg-olympus-primary/20',
...
const iconContainerClasses = computed(() => [
  'rounded-lg flex items-center justify-center transition-transform duration-150',
  sizeConfig[props.size].iconContainer,
])
...
const actionButtonClasses = (action: QuickAction, _index: number) => [
  ...
  'bg-olympus-bg hover:bg-olympus-elevated',
  'border border-olympus-border-subtle hover:border-olympus-border',
  'transition-all duration-200 text-left group outline-none',
  'focus-visible:ring-2 focus-visible:ring-olympus-primary/50',
  'focus-visible:ring-offset-2 focus-visible:ring-offset-olympus-surface',
  'active:scale-[0.99] hover:shadow-lg hover:shadow-olympus-primary/5',
  sizeConfig[props.size].gap,
  sizeConfig[props.size].padding,
  action.disabled && 'opacity-50 cursor-not-allowed',
  action.pinned && 'ring-1 ring-amber-500/30',
]
...
const shortcutClasses = computed(() => [
  'text-[10px] text-olympus-text-subtle bg-olympus-surface px-1.5 py-0.5 rounded',
  'border border-olympus-border font-mono',
  'opacity-0 group-hover:opacity-100 transition-opacity',
])

const arrowIconClasses = computed(() => [
  'w-4 h-4 text-olympus-text-subtle',
...
const showMoreButtonClasses = computed(() => [
  ...
  'text-xs text-olympus-text-muted hover:text-olympus-text',
  'transition-colors duration-150 rounded-lg hover:bg-olympus-bg',
])

// Recent section classes
const recentSectionClasses = computed(() => [
  'mt-4 pt-4 border-t border-olympus-border',
])
...
const modalClasses = computed(() => [
  'relative bg-olympus-surface border border-olympus-border rounded-2xl',
  'w-full max-w-md shadow-2xl',
])

// Customize item classes
const customizeItemClasses = computed(() => [
  'flex items-center gap-3 p-3 rounded-xl bg-olympus-bg border border-olympus-border',
])
...
const ActionSkeleton = defineComponent({
  ...
  setup() {
    return () => h('div', {
      class: 'flex items-center gap-3 p-3 rounded-lg bg-olympus-bg border border-olympus-border-subtle animate-pulse',
    }, [
      ...
      h(resolveComponent('SharedSkeleton'), { customClass: 'w-4 h-4' }),
    ])
  },
})
