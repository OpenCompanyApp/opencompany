      ...
      <div v-else class="max-w-3xl mx-auto">
        ...
        <Transition name="toc">
          <nav
            ...
            class="fixed right-8 top-32 w-48 hidden xl:block"
          >
            <div class="bg-olympus-surface/50 backdrop-blur-sm rounded-lg border border-olympus-border p-4">
              <h4 class="text-xs font-semibold text-olympus-text-muted uppercase tracking-wide mb-3">
                On this page
              </h4>
              <ul class="space-y-1">
                <li
                ...
                >
                  <button
                    type="button"
                    :class="[
                      'text-left w-full text-sm py-1 px-2 rounded transition-colors truncate',
                      activeHeadingId === item.id
                        ? 'text-olympus-primary bg-olympus-primary/10'
        ...
        <div :class="proseClasses">
          ...
          <Transition name="toolbar">
            <div
              ...
              class="fixed z-50 bg-olympus-elevated rounded-lg border border-olympus-border shadow-xl p-1 flex items-center gap-0.5"
              :style="{
            ...
            >
              <TooltipProvider :delay-duration="300">
                <Tooltip>
                  <TooltipTrigger as-child>
                    <button
                      type="button"
                      class="p-2 rounded-md hover:bg-olympus-surface transition-colors"
                      @click="handleHighlight"
                    >
                      <Icon name="ph:highlighter" class="w-4 h-4 text-yellow-400" />
                    </button>
                  </TooltipTrigger>
                ...
                <Tooltip>
                  <TooltipTrigger as-child>
                    <button
                      ...
                      @click="handleComment"
                    >
                      <Icon name="ph:chat-circle" class="w-4 h-4 text-olympus-text-muted" />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top" :side-offset="4">
              ...
              <div class="w-px h-4 bg-olympus-border mx-1" />
                ...
                <Tooltip>
                  <TooltipTrigger as-child>
                    <button
                      ...
                      @click="handleShare"
                    >
                      <Icon name="ph:share" class="w-4 h-4 text-olympus-text-muted" />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top" :side-offset="4">
          ...
          <TransitionGroup name="content-block" tag="div">
            <template v-for="(block, index) in parsedContent" :key="`${block.tag}-${index}`">
              ...
              >
                ...
                <button
                  type="button"
                  class="absolute -left-6 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-olympus-surface"
                  :aria-label="`Copy link to ${block.content}`"
                  @click="copyHeadingLink(block.content)"
                >
                  <Icon name="ph:link" class="w-4 h-4 text-olympus-text-muted" />
                </button>
              </component>
              ...
              <div
                ...
                class="my-6 overflow-x-auto rounded-lg border border-olympus-border"
              >
                <table class="w-full text-sm">
                  <thead class="bg-olympus-surface">
                    <tr>
                      <th
                        v-for="(header, hIdx) in block.headers"
                        :key="hIdx"
                        class="px-4 py-2 text-left font-medium text-olympus-text border-b border-olympus-border"
                      >
                        {{ header }}
                      </th>
                    </tr>
                  ...
                  <tbody>
                    <tr
                      ...
                      :key="rIdx"
                      class="border-b border-olympus-border last:border-0 hover:bg-olympus-surface/50 transition-colors"
                    >
                      <td
                        v-for="(cell, cIdx) in row"
                        :key="cIdx"
                        class="px-4 py-2 text-olympus-text"
                      >
                        {{ cell }}
                      </td>
                    </tr>
              ...
              >
                <button
                  type="button"
                  :class="[
                    'mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-all duration-150',
                    block.checked
                      ? 'bg-olympus-primary border-olympus-primary'
                      : 'border-olympus-border hover:border-olympus-text-muted',
                  ]"
                ...
                <span :class="[block.checked && 'line-through text-olympus-text-muted']">
                  {{ block.content }}
                </span>
              </div>
              ...
              <hr
                ...
                class="my-8 border-0 h-px bg-gradient-to-r from-transparent via-olympus-border to-transparent"
              />
              ...
              <figure
                ...
                class="my-6"
              >
                <div class="relative rounded-lg overflow-hidden border border-olympus-border">
                  <img
                    :src="block.src"
                    :alt="block.alt || ''"
                    class="w-full h-auto"
                  ...
                  <button
                    type="button"
                    class="absolute top-2 right-2 p-2 rounded-lg bg-olympus-bg/80 backdrop-blur-sm hover:bg-olympus-surface transition-colors opacity-0 hover:opacity-100"
                    @click="$emit('image-click', block.src)"
                  >
                    <Icon name="ph:arrows-out" class="w-4 h-4 text-olympus-text-muted" />
                  </button>
                </div>
                <figcaption
                  v-if="block.caption"
                  class="text-center text-sm text-olympus-text-muted mt-2 italic"
                >
                  {{ block.caption }}
                </figcaption>
              </figure>
              ...
              >
                ...
                <template v-if="block.inlineContent">
                  <template v-for="(part, pIdx) in block.inlineContent" :key="pIdx">
                    ...
                    <code
                      v-else-if="part.code"
                      class="px-1.5 py-0.5 rounded bg-olympus-surface text-olympus-primary font-mono text-sm"
                    >{{ part.text }}</code>
                    <a
                      v-else-if="part.link"
                      :href="part.link"
                      class="text-olympus-primary hover:underline"
                      target="_blank"
          ...
          <Transition name="progress">
            <div
              ...
              class="fixed bottom-8 right-8 w-12 h-12 rounded-full bg-olympus-elevated border border-olympus-border shadow-lg flex items-center justify-center"
            >
              <svg class="w-10 h-10 -rotate-90">
                ...
                <circle
                  ...
                  stroke-width="2"
                  fill="none"
                  class="text-olympus-primary"
                  :stroke-dasharray="circumference"
                  :stroke-dashoffset="circumference - (readProgress / 100) * circumference"
                  stroke-linecap="round"
                />
              </svg>
              <span class="absolute text-xs font-medium text-olympus-text">
...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from 'reka-ui'
...
const ContentSkeleton = defineComponent({
  ...
  setup() {
    return () => h('div', { class: 'space-y-6 animate-pulse' }, [
      // Title skeleton
      h('div', { class: 'h-8 bg-olympus-surface rounded-lg w-3/4' }),
      // Paragraphs
      h('div', { class: 'space-y-3' }, [
        h('div', { class: 'h-4 bg-olympus-surface rounded w-full' }),
        h('div', { class: 'h-4 bg-olympus-surface rounded w-5/6' }),
        h('div', { class: 'h-4 bg-olympus-surface rounded w-4/5' }),
      ]),
      // Subheading
      h('div', { class: 'h-6 bg-olympus-surface rounded-lg w-1/2 mt-8' }),
      // More paragraphs
      h('div', { class: 'space-y-3' }, [
        h('div', { class: 'h-4 bg-olympus-surface rounded w-full' }),
        h('div', { class: 'h-4 bg-olympus-surface rounded w-3/4' }),
      ]),
      // Code block skeleton
      h('div', { class: 'h-32 bg-olympus-surface rounded-xl mt-6' }),
      // More content
      h('div', { class: 'space-y-3 mt-6' }, [
        h('div', { class: 'h-4 bg-olympus-surface rounded w-full' }),
        h('div', { class: 'h-4 bg-olympus-surface rounded w-2/3' }),
      ]),
    ])
  },
})
...
const blockquoteVariants: Record<BlockquoteVariant, string> = {
  note: 'border-blue-400 bg-blue-500/5',
  tip: 'border-green-400 bg-green-500/5',
  warning: 'border-amber-400 bg-amber-500/5',
  danger: 'border-red-400 bg-red-500/5',
  info: 'border-olympus-primary bg-olympus-primary/5',
}
...
const blockquoteIconClasses: Record<BlockquoteVariant, string> = {
  note: 'w-4 h-4 text-blue-400',
  tip: 'w-4 h-4 text-green-400',
  warning: 'w-4 h-4 text-amber-400',
  danger: 'w-4 h-4 text-red-400',
  info: 'w-4 h-4 text-olympus-primary',
}
...
const proseClasses = computed(() => [
  'prose prose-invert prose-olympus max-w-none',
...
const blockClasses: Record<string, string> = {
  ...
  h2: 'text-2xl font-semibold mb-4 mt-8 text-olympus-text',
  h3: 'text-xl font-semibold mb-3 mt-6 text-olympus-text',
  h4: 'text-lg font-medium mb-2 mt-4 text-olympus-text',
  h5: 'text-base font-medium mb-2 mt-4 text-olympus-text',
  h6: 'text-sm font-medium mb-2 mt-4 text-olympus-text-muted uppercase tracking-wide',
  p: 'text-olympus-text leading-relaxed mb-4',
  li: 'text-olympus-text leading-relaxed ml-6 mb-2 list-disc',
  ol: 'text-olympus-text leading-relaxed ml-6 mb-4',
  ul: 'text-olympus-text leading-relaxed ml-6 mb-4',
  blockquote: 'border-l-4 border-olympus-primary/50 pl-4 py-2 my-4 rounded-r-lg',
}
...
.prose :deep(a) {
  @apply text-olympus-primary hover:underline transition-colors;
}

.prose :deep(strong) {
  @apply text-olympus-text font-semibold;
}

.prose :deep(em) {
  @apply text-olympus-text-muted;
}
...
@keyframes highlight-pulse {
  ...
  50% {
    background-color: oklch(var(--olympus-primary) / 0.3);
  }
}
