  ...
  <div :class="containerClasses">
    ...
    <div v-if="showHeader" :class="headerClasses">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p v-if="!loading" :class="subtitleClasses">
            ...
            <span v-if="selectedCount > 0" class="text-olympus-primary">
      ...
      <div class="flex items-center gap-2">
        ...
        <TooltipProvider v-if="showSearch" :delay-duration="300">
          <TooltipRoot>
            <TooltipTrigger as-child>
              <button
                type="button"
                :class="[headerButtonClasses, searchExpanded && 'bg-olympus-primary/10 text-olympus-primary']"
        ...
        <!-- View Toggle -->
        <div v-if="showViewToggle" class="flex items-center gap-0.5 p-0.5 rounded-lg bg-olympus-bg border border-olympus-border">
          <button
    ...
    <Transition name="slide-down">
      <div v-if="selectable && selectedCount > 0" :class="bulkActionsClasses">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm text-olympus-text-secondary">
            {{ selectedCount }} selected
          </span>
        </div>
    ...
    <div v-if="showFooter && !loading && documents.length > 0" :class="footerClasses">
      <div class="flex items-center gap-4 text-xs text-olympus-text-muted">
...
const headerIconClasses = computed(() => [
  'rounded-xl flex items-center justify-center bg-olympus-primary/20',
...
const headerButtonClasses = computed(() => [
  'flex items-center gap-1 px-2.5 py-1.5 rounded-lg transition-colors duration-150',
  'text-olympus-text-muted hover:text-olympus-text-secondary',
...
const searchContainerClasses = computed(() => [
  'px-4 py-2 border-b border-olympus-border-subtle',
])

// Bulk actions classes
const bulkActionsClasses = computed(() => [
  'flex items-center justify-between px-4 py-2',
  'bg-olympus-primary/5 border-b border-olympus-primary/20',
])
...
const footerClasses = computed(() => [
  'flex items-center justify-between px-4 py-2 border-t border-olympus-border-subtle',
])
      ...
      if (props.view === 'grid') {
        return h('div', { class: 'grid grid-cols-2 lg:grid-cols-3 gap-3' },
          Array.from({ length: props.count }).map((_, i) =>
            h('div', { key: i, class: 'p-4 rounded-xl border border-olympus-border-subtle animate-pulse' }, [
