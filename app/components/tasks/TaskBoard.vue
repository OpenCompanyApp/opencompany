  ...
  >
    ...
    >
      <div class="flex items-center gap-3">
        <h2 :class="['font-semibold text-olympus-text', sizeConfig[size].title]">
          {{ title }}
        </h2>
        <span
          v-if="totalTasks > 0"
          :class="[
            'font-medium bg-olympus-surface text-olympus-text-muted rounded-full',
      ...
      <div :class="['flex items-center', sizeConfig[size].statsGap]">
        ...
        >
          ...
          <span class="text-olympus-text-muted">{{ stat.count }}</span>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <div
          v-if="showLeftShadow"
          class="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-olympus-elevated to-transparent pointer-events-none z-10"
        />
      </Transition>
      ...
      >
        ...
        <div
          v-if="allowAddColumn"
          :class="[
            'flex flex-col items-center justify-center shrink-0 border-2 border-dashed border-olympus-border rounded-2xl',
            'hover:border-olympus-primary hover:bg-olympus-primary/5 transition-all duration-200 cursor-pointer group',
        ...
        >
          <Icon
            name="ph:plus"
            :class="[
              'text-olympus-text-muted group-hover:text-olympus-primary transition-colors',
              sizeConfig[size].addColumnIcon
            ]"
          />
          <span
            :class="[
              'text-olympus-text-muted group-hover:text-olympus-primary transition-colors font-medium mt-2',
      ...
      >
        <div
          ...
          class="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-olympus-elevated to-transparent pointer-events-none z-10"
        />
      </Transition>
    </div>
    ...
    >
      <div
        v-if="loading"
        class="absolute inset-0 bg-olympus-elevated/50 backdrop-blur-sm flex items-center justify-center z-20"
      >
        <div class="flex flex-col items-center gap-3">
          <div class="relative">
            <div class="w-10 h-10 border-3 border-olympus-border rounded-full" />
            <div class="absolute inset-0 w-10 h-10 border-3 border-olympus-primary border-t-transparent rounded-full animate-spin" />
          </div>
          <span class="text-sm text-olympus-text-muted">Loading tasks...</span>
        </div>
      </div>
    </Transition>
    ...
    >
      ...
      >
        ...
        <h3 :class="['font-semibold text-olympus-text mb-1', sizeConfig[size].emptyTitle]">
          No tasks yet
        </h3>
        <p :class="['text-olympus-text-muted max-w-sm', sizeConfig[size].emptyDescription]">
          Create your first task to get started with your project board
        </p>
        <button
          :class="[
            'mt-4 flex items-center gap-2 bg-olympus-primary text-white rounded-xl font-medium',
            'hover:bg-olympus-primary-hover transition-colors',
    ...
    >
      <div
        ...
        :class="[
          'absolute flex items-center gap-4 bg-olympus-surface/90 backdrop-blur-sm border border-olympus-border rounded-xl',
      ...
      >
        <div class="flex items-center gap-1.5">
          <kbd
            :class="[
              'font-mono bg-olympus-elevated border border-olympus-border rounded',
              sizeConfig[size].kbd
            ]"
          >
            N
          </kbd>
          <span class="text-olympus-text-muted">New task</span>
        </div>
        <div class="flex items-center gap-1.5">
          <kbd
            :class="[
          ...
          <span class="text-olympus-text-muted">Move task</span>
        </div>
        <div class="flex items-center gap-1.5">
          <kbd
            :class="[
...
const columns: Column[] = [
  ...
  { status: 'in_progress', title: 'In Progress', icon: 'ph:circle-half', color: 'bg-olympus-primary' },
  { status: 'done', title: 'Done', icon: 'ph:check-circle', color: 'bg-olympus-success' },
]
