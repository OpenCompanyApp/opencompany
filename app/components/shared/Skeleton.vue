<template>
  ...
  <div v-if="preset" :class="wrapperClasses">
    ...
    <template v-else-if="preset === 'table'">
      <div class="space-y-2">
        <!-- Header -->
        <div class="flex gap-4 pb-2 border-b border-olympus-border">
          <SkeletonItem
    ...
    <template v-else-if="preset === 'stats'">
      <div :class="['grid gap-4', gridCols]">
        <div
          v-for="i in count"
          :key="i"
          class="p-4 rounded-xl bg-olympus-surface/50 space-y-2"
        >
          <SkeletonItem
            variant="text"
            width="60%"
...
.skeleton-wave {
  background: linear-gradient(
    ...
    oklch(var(--color-olympus-elevated) / 0.9) 50%,
    oklch(var(--color-olympus-surface) / 0.8) 100%
  );
...
.skeleton-shimmer {
  background: linear-gradient(
    110deg,
    oklch(var(--color-olympus-surface) / 0.8) 8%,
    oklch(var(--color-olympus-elevated) / 1) 18%,
    oklch(var(--color-olympus-surface) / 0.8) 33%
  );
  background-size: 200% 100%;
