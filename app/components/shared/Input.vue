<template>
  <div :class="wrapperClasses">
    ...
    >
      ...
      <span v-if="optional" class="text-olympus-text-muted ml-1 text-xs font-normal">(optional)</span>
    </label>
    ...
    <div v-if="error || hint || showCounter" class="flex items-start justify-between gap-2">
      ...
      <span
        ...
        :class="[
          'text-xs shrink-0',
          isOverLimit ? 'text-red-400' : 'text-olympus-text-muted',
        ]"
      >
...
const variantClasses = computed(() => {
  ...
  const variants: Record<InputVariant, string> = {
    default: `${base} bg-olympus-surface border rounded-lg`,
    filled: `${base} bg-olympus-elevated border-0 rounded-lg hover:bg-olympus-surface`,
    outlined: `${base} bg-transparent border-2 rounded-lg`,
    ghost: `${base} bg-transparent border-0 hover:bg-olympus-surface/50 rounded-lg`,
    underlined: `${base} bg-transparent border-0 border-b-2 rounded-none`,
  }
...
const borderStateClasses = computed(() => {
  if (props.error) {
    return 'border-red-500/50 focus-within:border-red-500 focus-within:ring-red-500/20'
  }
  if (props.success) {
    return 'border-green-500/50 focus-within:border-green-500 focus-within:ring-green-500/20'
  }
  if (isFocused.value) {
    return 'border-olympus-primary focus-within:ring-olympus-primary/20'
  }
  return 'border-olympus-border hover:border-olympus-border-hover focus-within:border-olympus-primary focus-within:ring-olympus-primary/20'
})
...
const labelClasses = computed(() => [
  'text-sm font-medium text-olympus-text flex items-center',
...
const containerClasses = computed(() => [
  ...
  props.variant !== 'ghost' && props.variant !== 'underlined' && 'focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-olympus-bg',
])

// Input classes
const inputClasses = computed(() => [
  'w-full bg-transparent outline-none text-olympus-text placeholder:text-olympus-text-subtle',
...
const floatingLabelClasses = computed(() => {
  ...
  return [
    'absolute left-0 transition-all duration-200 pointer-events-none text-olympus-text-muted',
    // Positioned state (when focused or has value)
    (isFocused.value || hasValue)
      ? '-top-6 text-xs text-olympus-primary'
      : 'top-1/2 -translate-y-1/2 text-sm',
    // Peer focus (CSS fallback)
    'peer-focus:-top-6 peer-focus:text-xs peer-focus:text-olympus-primary peer-focus:translate-y-0',
...
const prefixClasses = computed(() => [
  'flex items-center text-olympus-text-muted shrink-0',
  sizeClasses[props.size].text,
  props.variant === 'default' && '-ml-1 pr-2 border-r border-olympus-border mr-2',
])
...
const suffixClasses = computed(() => [
  'flex items-center text-olympus-text-muted shrink-0',
  sizeClasses[props.size].text,
  props.variant === 'default' && '-mr-1 pl-2 border-l border-olympus-border ml-2',
])

// Left icon classes
const leftIconClasses = computed(() => [
  'flex items-center justify-center text-olympus-text-muted shrink-0',
])
