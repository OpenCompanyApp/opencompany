...
class UpdateAutomation implements Tool
{
    ...
    public function handle(Request $request): string
    {
        try {
            ...
            if ($targetType === 'script' && isset($request['script'])) {
                $updates = [...$updates, ...app(ScriptAdmission::class)->admit((string) $request['script'])];
            } elseif ($targetType === 'script' && (bool) ($request['isActive'] ?? $automation->is_active)
                && ! app(ScriptAdmission::class)->matches($automation)) {
                return 'Submit and validate a Ruby body before enabling this legacy or stale automation.';
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'script' => $schema
                ->string()
                ->description('New synchronous Ruby script for a script automation. It is compiled before saving.'),
