...
- No migration has run against the primary application database and no primary
  domain, worker, extension configuration, or PHP service has been switched.
  The user selected a separate test site/database: `opencompany-mruby.test` now
  has real PHP-FPM/browser and bounded database-worker evidence against fresh
  synthetic data. There are no live provider calls in these checks.
...
the primary local web/queue checkout has not been switched. Previous candidates are
retained for recovery.

After the RC artifact installation, the combined focused application run passes
123 tests / 3,035 assertions, including real queue execution, stale/redelivered queue fencing,
...
The separately authorized [local test site](../testing/mruby-local-site.md) now
adds authenticated, real PHP-FPM validation/execution and a real SQLite queue
run returning `42`. The one script run made zero capability calls; its local
notification follow-ups were inspected and drained. The test automation is
disabled and no worker remains running. Its fresh database and synthetic source
are retained with a private backup; this is not a real-user migration or primary
site cutover. This browser pass found and fixed the console's null-error handling;
the new console fixture and existing history fixture are now wired into CI.
...
`docs/INDEX.md` links this evidence log, the implementation plan, browser
fixtures and the separate real local-site qualification record.

## Remaining gates — do not claim these are shipped

1. Implement schema-aware static API validation from an actual Ruby parser/AST
...
2. Establish a matched-model AX task corpus and baselines; current editor and
   agent-experience tests are not a matched-model measurement.
3. Complete native parser/compiler/value fuzzing, sustained cancellation/memory
...
4. Replace remaining integration path coupling with released immutable dependencies,
   address the triaged baseline Composer advisories (29 across nine existing packages),
   ...
   The selected isolated Mac site/worker qualification is delivered. Any later
   primary-site or production cutover still requires its own recoverable source
   export, worker drain and explicit authority; it is not implicitly included
   in the user's choice to keep VFS/run-control separate.
